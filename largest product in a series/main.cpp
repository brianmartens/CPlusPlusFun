#include <iostream>
#include <fstream>
#include <vector>
using namespace std;

bool nullcheck(const size_t *G, size_t index)
{
    bool condition = true;
    for(size_t j=index ; j<(index+13) ; j++)
    {
        cout << *(G + j);
        if(*(G + j)==0) {
            condition = false;
            cout << " contained a zero" << endl;
            return condition;
        }
    }
    cout << " was valid" << endl;
    return condition;
}

size_t maximum(vector<size_t> &G)
{
    size_t j=0;
    for(size_t i=0 ; i<G.size() ; i++)
    {
        if(G.at(i)>j) j=G.at(i);
    }
    return j;
}

int main()
{
    ofstream myfile;
    myfile.open("numberstream.txt");
    const char* number = "";
    size_t numberArr[1000];
    vector<size_t> products;
    for(size_t i=0 ; i<1000 ; i++)
    {
        numberArr[i] = *(number + i) - '0'; // put numbers into size_t array
    }
    for(size_t i=0 ; i<1000 ; i++)
    {
        if((nullcheck(numberArr, i)))
        {
            size_t tempProduct = 1;
            for(size_t j=i ; j<(i+13) ; j++)
            {
                if(*(number + j)=='\0')
                {
                    break;
                }
                tempProduct *= numberArr[j];
            }
            products.push_back(tempProduct);
            myfile << tempProduct << endl;
        }
    }
    myfile << endl << maximum(products);
    return 0;
}
